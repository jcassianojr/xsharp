...
METHOD Start() 
	LOCAL oWindow AS pcpshell //StandardShellWindow
 ...
 //   EnableAppVisualTheme(TRUE)
  ...
  //  cHELP+="pcp.chm"       //hlp nao funciona em versao novos sistema operacional migrados para chm
    IF File(cHELP)
...
ELSE
   ...
   IF ! aLOGIN[1]
    ...
    ZNOMEFOLHA:=aLOGIN[5]	
   	...
   	oWINDOW:Caption := "Modulo PCP  Empresa: "+StrZero(Zempresa,3)+" Competencia: "+StrZero(Zmes,2)+"/"+StrZero(Zano,4)


    IF aLOGIN[1]
       GRAVALOG("ACESSOU","ENT","PCP")
       oMAIL:=XJMAIL{SELF,.T.}
       oMAIL:SHOW()		
       oWindow:Show(SHOWZOOMED)//	oWindow:Show()		
