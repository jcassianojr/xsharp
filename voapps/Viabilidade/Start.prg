...
METHOD Start() 
	LOCAL oWindow AS VIASHELL //StandardShellWindow
  ...
  //   DynSize(250)                            // increase the initial GC allocation to 12MB
   //  SetMaxRegisteredAxitMethods(64000) //Standard is 16000
	...
	oWindow := VIASHELL{SELF}
	
  cHELP:=PEGINIVAL(ZCURINI,"HELP","CAMINHO")
//    cHELP+="viabili.chm"       //hlp nao funciona em versao novos sistema operacional migrados para chm
    IF File(cHELP)
	...
	oWINDOW:Caption := "Modulo Viabilidade"
...
ELSE
   ...
   IF ! aLOGIN[1]
    ...
    ZNOMEFOLHA:=aLOGIN[5]
   	...
   	oWINDOW:Caption := "Modulo Viabilidade  Empresa: "+StrZero(Zempresa,3)+" Competencia: "+StrZero(Zmes,2)+"/"+StrZero(Zano,4)



    IF aLOGIN[1]

       GRAVALOG("ACESSOU","ENT","VIA")
       oMAIL:=XJMAIL{SELF,.T.}
       oMAIL:SHOW()		
