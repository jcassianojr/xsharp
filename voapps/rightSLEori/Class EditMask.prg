...
GLOBAL CONST VK_INSERT := 0X2D AS LONG
...
//	SELF:oEditOwner:Text := SELF:sValue
	SELF:oEditOwner:SetText(SELF:sValue)        // wcm 2011-10-25
...
METHOD IsEditPos(iPos AS long) AS LOGIC 
	...
	ELSE
		...
		ENDIF
		IF lRet
			lRet := IsEditTemplChar(SELF:sTemplate:Substring(iPos-1,1))
		ENDIF
	ENDIF
	RETURN lRet
...
METHOD DeleteSelection(iStart AS LONG, iEnd AS LONG) AS VOID 
	...
	ELSE
		lInvert := ((lNumeric) .AND. ((sValue:IndexOf(self:cDecSep,iEnd) +1) > 0))
		IF !lInvert
			iDelPos := iStart
...
//	SELF:oEditOwner:Text            := cText //SELF:sValue
	SELF:oEditOwner:SetText(cText)                              // wcm 2011-10-25
...
/****************************************************************************/
METHOD ProcessKeyEvent(nKeyCode AS LONG,lCmdKey AS LOGIC) AS LOGIC
	LOCAL lRet      AS LOGIC
	LOCAL lInvert   AS LOGIC
	LOCAL iStart    AS LONG
	LOCAL iEnd      AS LONG
	LOCAL cChar     AS STRING
//    local cKeyName  AS String
//    local kc        AS KeysConverter 
	
//    kc := KeysConverter{}
//    cKeyName := kc:ConvertToString(nKeycode)
	cChar := ChrW((DWORD)nKeyCode)    
	lRet    := TRUE
	
	IF lcmdKey
		DO CASE
			CASE nKeyCode == Keys.Left
				oEditOwner:__CurPos := SELF:PrevEditPos(-1)
			CASE nKeyCode == Keys.Right
				oEditOwner:__CurPos := SELF:NextEditPos(-1)            
			CASE nKeyCode == Keys.Home
				oEditOwner:__CurPos := SELF:NextEditPos(1)
			CASE nKeyCode == Keys.End
				oEditOwner:__CurPos := SELF:PrevEditPos(SELF:sValue:TrimEnd():Length) + 1
			CASE nKeyCode == Keys.Delete .or. nKeyCode == Keys.Back
				iStart  := SELF:oEditOwner:__SavedCurPos 
				iEnd    := iStart + SELF:oEditOwner:__SavedCurLen
				IF iStart <> iEnd .and. (nKeycode == Keys.Delete .or. nKeyCode == Keys.Back)
					SELF:DeleteSelection(iStart, iEnd)
				ELSE
					IF nKeyCode == Keys.Back
						oEditOwner:__CurPos := SELF:PrevEditPos(-1)
					ENDIF
					...
					ENDIF
				ENDIF
			CASE cChar == "," .or.  nKeyCode == Keys.OemPeriod .or. nKeyCode == Keys.Decimal
				iStart  := SELF:oEditOwner:__SavedCurPos 
				iEnd    := iStart + SELF:oEditOwner:__SavedCurLen
				IF iStart <> iEnd 
					SELF:DeleteSelection(iStart, iEnd)
...
METHOD ProcessChar(cChar AS STRING) AS LOGIC 
		...
		ELSE
			...
			ELSE
				...
				ELSE
					...
					IF !Int32.TryParse(cTempValue,nResult)
	...
	IF SELF:MatchesTemplChar(cChar, cTemplChar, FALSE)
		IF cTemplChar == "!"
			cChar:ToUpper()
		ENDIF
		IF ((SELF:wOverWrite == overwriteBehavior.Always) .or. (SELF:wOverWrite == overwriteBehavior.Onkey .and. IsOverWriteModeEnabled())) .or.;
...
//   LOCAL bKeyStroke AS BYTE
   aKeyStates := BYTE[]{256}
   GetKeyboardState(aKeyStates)
//   bKeyStroke := aKeyStates[VK_INSERT]

//   lResult := !LOGIC(_CAST, _And(bKeyStroke, 1))
...
METHOD MatchesTemplChar(cTest AS STRING, cTemplChar AS STRING, lIgnoreBlank AS LOGIC) AS LOGIC
	...
	cTemplChar  := cTemplChar:ToUpper()             // 20120216 - wcm - Template characters are always uppercase so upper the string before we test
...
PROPERTY TypeasString AS STRING
	GET
		...
		DO CASE
			CASE SELF:nType == SLEType.Numeric
				cResult := "N"
			CASE SELF:nType == SLEType.Character
				cResult := "C"
			CASE SELF:nType == SLEType.Date
				cResult := "D"
			CASE SELF:nType == SLEType.Logic
				cResult := "L"
			OTHERWISE
				cResult := "C"
		END CASE
