#region DEFINES
DEFINE     COMPRESSION_ADOBE_DEFLATE   := 0x0400       // Deflate compression, as recognized by Adobe
DEFINE	    COMPRESSION_CCITTFAX3	:= 0x1000	// CCITT Group 3 fax encoding
DEFINE	    COMPRESSION_CCITTFAX4	:= 0x2000	// CCITT Group 4 fax encoding
DEFINE	    COMPRESSION_DEFLATE		:= 0x0200	// Deflate compression
DEFINE	    COMPRESSION_JPEG		:= 0x8000	// %JPEG DCT compression
DEFINE	    COMPRESSION_LZW		:= 0x4000       // Lempel-Ziv  & Welch
DEFINE	    COMPRESSION_NONE		:= 0x0800	// dump mode
DEFINE	    COMPRESSION_PACKBITS	:= 0x0100	// Macintosh RLE
DEFINE EXIF_MODEL_ANIMATION	:= 9	// Animation metadata
DEFINE EXIF_MODEL_COMMENTS		:= 0	// single comment or keywords
DEFINE EXIF_MODEL_CUSTOM		:= 10	// Used to attach other metadata types to a dib
DEFINE EXIF_MODEL_EXIF			:= 2	// Exif-specific metadata
DEFINE EXIF_MODEL_GEOTIFF		:= 8	// GeoTIFF metadata
DEFINE EXIF_MODEL_GPS			:= 3	// Exif GPS metadata
DEFINE EXIF_MODEL_INTEROP		:= 5	// Exif interoperability metadata
DEFINE EXIF_MODEL_IPTC			:= 6	// IPTC/NAA metadata
DEFINE EXIF_MODEL_MAIN			:= 1	// Exif-TIFF metadata
DEFINE EXIF_MODEL_MAKERNOTE 	:= 4	// Exif maker note metadata
DEFINE EXIF_MODEL_NONE			:= -1	// No Specified EXIF Model, so use MAIN and EXIF
DEFINE EXIF_MODEL_XMP			:= 7	// Abobe XMP metadata
...
FUNCTION CAPaintIsLoaded() AS LOGIC PASCAL
...
FUNCTION FreeCAPaint() AS LOGIC PASCAL
// DLL is now referenced statically
RETURN TRUE


FUNCTION InitializeCAPaint() AS LOGIC PASCAL
// DLL is now referenced statically, so always TRUE
// Same as standard CAPaint
RETURN TRUE
