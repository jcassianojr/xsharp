...
METHOD Start() 
	LOCAL oWindow AS proshell //StandardShellWindow
 ...
 //   EnableAppVisualTheme(TRUE)
  ...
  //   SetMaxRegisteredAxitMethods(64000) //Standard is 16000
    ...
    ZANO:=Year(Today())
 	...
 	oWindow := proshell{SELF}


  cHELP:=PEGINIVAL(ZCURINI,"HELP","CAMINHO")
 //   cHELP+="producao.chm"       //hlp nao funciona em versao novos sistema operacional migrados para chm
    IF File(cHELP)
	...
	oWINDOW:Caption := "Modulo de Producao"
...
ELSE
   ...
   IF ! aLOGIN[1]
    ...
    ZNOMEFOLHA:=aLOGIN[5]	
   	...
   	oWINDOW:Caption := "Modulo Producao  Empresa: "+StrZero(Zempresa,3)+" Competencia: "+StrZero(Zmes,2)+"/"+StrZero(Zano,4)



    IF aLOGIN[1]
       GRAVALOG("ACESSOU","ENT","PRO")
       oMAIL:=XJMAIL{SELF,.T.}
       oMAIL:SHOW()		
