...
METHOD Start() 
	LOCAL oWindow AS rhshell //StandardShellWindow
 ...
 //   EnableAppVisualTheme(TRUE)
  ...
  //   SetMaxRegisteredAxitMethods(64000) //Standard is 16000
	...
	oWindow := rhshell{SELF}
	
  cHELP:=PEGINIVAL(ZCURINI,"HELP","CAMINHO")
  //  cHELP+="rh.chm"       //hlp nao funciona em versao novos sistema operacional migrados para chm
    IF File(cHELP)
...
//    alert(ZDIRE)
    ...
    zWRPT:=aLOGIN[4]
    ZNOMEFOLHA:=aLOGIN[5]
   	oWINDOW:Caption := "Modulo RH  Empresa: "+StrZero(Zempresa,3)+" Competencia: "+StrZero(Zmes,2)+"/"+StrZero(Zano,4)
    IF aLOGIN[1]
       GRAVALOG("ACESSOU","ENT","RH")
       oMAIL:=XJMAIL{SELF,.T.}
       oMAIL:SHOW()		
