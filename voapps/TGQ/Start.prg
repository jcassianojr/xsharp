...
METHOD Start() 
	LOCAL oWindow AS TGQSHELL //StandardShellWindow
 ...
 //   EnableAppVisualTheme(TRUE)
  ...
  //   SetMaxRegisteredAxitMethods(64000) //Standard is 16000
	...
	oWindow := TGQSHELL{SELF}
	
  cHELP:=PEGINIVAL(ZCURINI,"HELP","CAMINHO")
 //   cHELP+="tgq.chm"       //hlp nao funciona em versao novos sistema operacional migrados para chm
    IF File(cHELP)
...
ELSE
   ...
   IF ! aLOGIN[1]
    ...
    zWRPT:=aLOGIN[4]
    ZNOMEFOLHA:=aLOGIN[5]
   	oWINDOW:Caption := "Modulo TGQ: "+StrZero(Zempresa,3)+" Competencia: "+StrZero(Zmes,2)+"/"+StrZero(Zano,4)

    IF aLOGIN[1]
       GRAVALOG("ACESSOU","ENT","TGQ")
       oMAIL:=XJMAIL{SELF,.T.}
       oMAIL:SHOW()		
