...
GLOBAL lSECBCO AS LOGIC

GLOBAL ZCOMP AS STRING

GLOBAL ZDIR AS STRING

GLOBAL ZDIRE AS STRING

GLOBAL ZDIRN AS STRING

GLOBAL zDUSO AS DATE

GLOBAL ZREL AS DWORD
...
METHOD Start() 
	LOCAL oWindow AS ptoshell //StandardShellWindow
 ...
 //   EnableAppVisualTheme(TRUE)
  ...
  //   SetMaxRegisteredAxitMethods(64000) //Standard is 16000
	...
	oWindow := ptoshell{SELF}
	
  cHELP:=PEGINIVAL(ZCURINI,"HELP","CAMINHO")
//    cHELP+="folhapto.chm"       //hlp nao funciona em versao novos sistema operacional migrados para chm
    IF File(cHELP)
...
ELSE
 aDAD:={ZCURINI,"MUSER.DBF",zCURDIR}
   ...
   IF ! aLOGIN[1]
    ...
    zWRPT:=aLOGIN[4]
    ZNOMEFOLHA:=aLOGIN[5]
   	oWINDOW:Caption := "Modulo Ponto  Empresa: "+StrZero(Zempresa,3)+" Competencia: "+StrZero(Zmes,2)+"/"+StrZero(Zano,4)


    IF  aLOGIN[1]
        GRAVALOG("ACESSOU","ENT","PTO")
        oMAIL:=XJMAIL{SELF,.T.}
        oMAIL:SHOW()		
